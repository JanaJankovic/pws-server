...
function isWinterSeason(currentMonth, growMonth, winterMonth) {
  if (winterMonth == 0) return false;

  if (growMonth > winterMonth)
    return currentMonth >= winterMonth && currentMonth < growMonth;
  else return !(currentMonth >= growMonth && currentMonth < winterMonth);
}

function isWateringTime(
  id,
  res,
  frequency,
  frequency_modifier,
  ...
  const interval = Math.floor(
    new Date(date.getFullYear(), date.getMonth() + 1, 0).getDate() / f
  );
  let last = counter * interval;
  ...
  return current % interval == 0 && last < current;
}
...
module.exports = {
  ...
  create: function (req, res, next) {
    if (
      typeof req.body.relay_pin == "undefined" &&
      !req.body.relay_pin &&
      typeof req.body.moisture_pin == "undefined" &&
      !req.body.moisture_pin &&
      typeof req.body.byte_address == "undefined" &&
      !req.body.byte_address &&
      typeof req.body.plant._id == "undefined" &&
      !req.body.plant._id
    ) {
    ...
    UserModel.findOne({ _id: id }, function (err, user) {
      ...
      PlantModel.findOne({ _id: req.body.plant._id }, function (err, plant) {
        ...
        if (!plant) {
          const err = new Error("No such plant");
          err.status = 404;
          return next(err);
        }
  ...
  addDateLog: function (req, res) {
    ...
    RecipientModel.findOne({ _id: id }, function (err, recipient) {
      ...
      recipient.water_log.push(
        formatter.format(new Date(), "DD/MM/YYYY HH:mm:ss")
      );
      recipient.counter =
        req.body.message == "Schedule"
          ? recipient.counter + 1
          : recipient.counter;
  ...
  list: function (req, res) {
    const id = req.params.user_id;
    const address = req.params.byte_address;
    ...
    RecipientModel.aggregate([
      ...
      for (let i = 0; i < rs.length; i++) {
        var isWater = isWateringTime(
          rs[i]._id,
          res,
          rs[i].frequency,
          rs[i].frequency_modifier,
          rs[i].counter,
          rs[i].growth_month,
          rs[i].hibernation_month
        );
        ...
        var r = {
          ...
          moisture: isWinterSeason(
            new Date().getMonth() + 1,
            rs[i].growth_month,
            rs[i].hibernation_month
          )
            ? rs[i].moisture - rs[i].moisture_modifier
            : rs[i].moisture,
          water: isWater,
        };
  ...
  update: function (req, res, next) {
    ...
    RecipientModel.findOne({ _id: id }, (err, recipient) => {
      if (err) {
        err.message = "Error when getting the recipient";
        err.status = 500;
        return next(err);
      ...
      if (!recipient) {
        const err = new Error("No such recipient");
        err.status = 404;
        return next(err);
      }

      recipient.plant_id =
        typeof req.body.plant != "undefined" && req.body.plant._id
          ? req.body.plant._id
          : recipient.plant_id;
      recipient.path = req.body.path ? req.body.path : recipient.path;
      recipient.byte_address = req.body.byte_address
        ? req.body.byte_address
        : recipient.byte_address;
      recipient.relay_pin = req.body.relay_pin
        ? req.body.relay_pin
        : recipient.relay_pin;
      recipient.moisture_pin = req.body.moisture_pin
        ? req.body.moisture_pin
        : recipient.moisture_pin;
      recipient.water_log = req.body.water_log
        ? req.body.water_log
        : recipient.water_log;
  ...
  remove: function (req, res, next) {
    ...
    RecipientModel.findOne({ _id: id }, function (err, recipient) {
      ...
      PlantModel.deleteOne(
        { _id: recipient.plant_id, customized: true },
        function (err) {
          if (err) {
